...
import Nav from "../Nav.jsx";   // ← relative path (was hardcoded Windows path before)
import { getCoins, getStreak, getCompletedTopics } from "../api.js";

// ── RPG levels — icon field added ─────────────────────────────────────────────
const RPG_LEVELS = [
...
const TOPIC_COLORS = { react:"#C4622D", js:"#C9952A", css:"#5A7A5C", node:"#8C7B6B" };
...
const GAP_META = {
  ...
  "CSS Grid":         { desc:"Two-dimensional layouts that Flexbox can't handle alone.",       time:"12 min", link:"https://css-tricks.com/snippets/css/complete-guide-grid/" },
...
function detectGaps(completed) {
  const gaps = new Set();
  completed.forEach(t => (GAP_SUGGESTIONS[t] || []).forEach(g => {
...
function KnowledgeGraph({ completedTopics }) {
  ...
  const nodes = KG_NODES.map(n => ({
    ...n,
    completed: n.completed || completedTopics.some(
      t => t.toLowerCase() === n.id.toLowerCase() || t.toLowerCase() === n.label.toLowerCase()
    ),
  }));
    ...
    <div>
      ...
      <div style={{ background:"#1A1412", borderRadius:14, overflow:"hidden", border:"1px solid #2C1810" }}>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:"block" }}>
          ...
          {nodes.map(n=>{
            const c=TOPIC_COLORS[n.topic], cx=nx(n), cy=ny(n);
            return (
              <g key={n.id}>
...
export default function Dashboard() {
  ...
  // Re-read coins whenever GrowMode earns some
  useEffect(() => {
    function sync() { setCoins(getCoins()); }
  ...
  useEffect(() => { setTopics(getCompletedTopics()); }, []);
        ...
        <div style={{ display:"flex", gap:20, marginBottom:20, alignItems:"flex-start" }}>
          ...
          <div style={{ flex:"0 0 280px", background:"white", border:"1px solid var(--border)", borderRadius:18, padding:24, animation:"fadeUp 0.5s ease both", position:"relative", overflow:"hidden" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20 }}>
              <div style={{ width:56, height:56, borderRadius:14, background:"#FDF0E8", border:"1.5px solid #F0C4A8", display:"flex", alignItems:"center", justifyContent:"center", fontSize:26 }}>
                {level.icon}
              </div>
              <div>
            ...
            <div style={{ background:"white", border:"1px solid var(--border)", borderRadius:14, padding:20, flex:1 }}>
              ...
              {actions.map(a => (
                <div key={a.id} className="aw-action"
                  ...
                  <span style={{ fontSize:13, color:a.done?"#B5A898":"#2C1810", flex:1, textDecoration:a.done?"line-through":"none" }}>{a.label}</span>
