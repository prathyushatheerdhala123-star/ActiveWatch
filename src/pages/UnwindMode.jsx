...
// ── emoji field added to every mood ──────────────────────────────────────────
const MOODS = [
...
export default function UnwindMode() {
  ...
  // Show wind-down nudge after 3 s (simulates 45 min of watching for demo)
  useEffect(() => {
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
            ...
            <div style={{ background:"white", border:"1px solid var(--border)", borderRadius:18, padding:22, animation:"fadeUp 0.4s 0.05s ease both" }}>
              ...
              {mood && (
                <div style={{ padding:"10px 14px", background:"#EEF5EE", border:"1px solid #C2D9C3", borderRadius:10, fontSize:13, color:"#5A7A5C" }}>
                  ✨ Queue updated to match your mood
                </div>
              )}
            </div>
            ...
            <div style={{ background:"white", border:"1px solid var(--border)", borderRadius:18, padding:22, animation:"fadeUp 0.4s 0.1s ease both" }}>
              ...
              <div style={{ display:"flex", gap:12 }}>
                <div style={{ flex:1, background:"#FDF5E6", border:"1px solid #E8D098", borderRadius:12, padding:14, textAlign:"center" }}>
                ...
                <div style={{ flex:1, background:"#EEF5EE", border:"1px solid #C2D9C3", borderRadius:12, padding:14, textAlign:"center" }}>
            ...
            <div style={{ background:"white", border:"1px solid var(--border)", borderRadius:18, padding:22, animation:"fadeUp 0.4s 0.15s ease both" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  ...
                  <p style={{ color:"#8C7B6B", fontSize:13, margin:0, fontStyle:"italic", maxWidth:200 }}>Videos won't autoplay. One tap to continue — always your call.</p>
                </div>
            ...
            <div style={{ background:"white", border:"1px solid var(--border)", borderRadius:18, padding:22, animation:"fadeUp 0.4s 0.2s ease both" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                {[["8h","watched"],["14","saved"],["3","chapters"]].map(([v,l]) => (
                  <div key={l} style={{ background:"#FAF7F2", borderRadius:10, padding:12, textAlign:"center" }}>
            ...
            <div style={{ background:"white", border:"1px solid var(--border)", borderRadius:18, padding:22, animation:"fadeUp 0.4s 0.08s ease both" }}>
              ...
              {queue.length === 0 ? (
                ...
                <div key={v.id} className="aw-queue-row"
                  ...
                  <button
                    onClick={() => setQueue(prev => prev.filter(x => x.id !== v.id))}
                    onMouseEnter={e => e.currentTarget.style.color="#C4622D"}
                    onMouseLeave={e => e.currentTarget.style.color="#D1C4B8"}
                    style={{ background:"none", border:"none", color:"#D1C4B8", fontSize:15, cursor:"pointer", padding:4, transition:"color 0.2s", lineHeight:1 }}>✕</button>
                </div>
              ))}
