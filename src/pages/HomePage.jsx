...
export default function HomePage() {
  ...
  async function handleSave() {
    ...
    if (!url.includes("youtube.com") && !url.includes("youtu.be")) {
      setError("That doesn't look like a YouTube link."); return;
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .aw-mode-pill:hover { border-color:#C4622D!important; transform:translateY(-2px); box-shadow:0 4px 16px rgba(196,98,45,0.12)!important; }
      ...
      <main style={{
        ...
        <div style={{ textAlign: "center", animation: "fadeUp 0.7s ease both", marginBottom: 52 }}>
          <div style={{
            display: "inline-block", background: "#FDF0E8", border: "1px solid #F0C4A8",
            color: "#C4622D", fontSize: 11, fontWeight: 600, padding: "5px 14px",
            borderRadius: 20, marginBottom: 28, letterSpacing: "0.07em", textTransform: "uppercase",
          }}>AI-Powered Learning</div>

          <h1 style={{
            fontFamily: "'Fraunces', serif",
            fontSize: "clamp(38px, 6vw, 62px)",
            fontWeight: 300, color: "#2C1810",
            lineHeight: 1.1, letterSpacing: "-1.5px", margin: "0 0 22px",
          }}>
            Watch smarter.<br />
            Learn <Typewriter words={["faster.", "deeper.", "actively.", "intentionally."]} />
          </h1>

          <p style={{ fontSize: 17, color: "#8C7B6B", lineHeight: 1.75, maxWidth: 480, margin: "0 auto", fontWeight: 300 }}>
            Paste any YouTube link. We figure out what kind of video it is
            and build the right experience around it — automatically.
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ flex: 1, position: "relative" }}>
              <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#B5A898", fontSize: 14, pointerEvents: "none" }}>▶</span>
              <input
                ref={inputRef}
                ...
                style={{
                  ...
                  padding: "13px 16px 13px 40px", color: "#2C1810", fontSize: 14,
            ...
            <button
              ...
              style={{
                ...
                padding: "13px 22px", color: "white", fontSize: 14, fontWeight: 500,
          ...
          {error && <p style={{ color: "#C4622D", fontSize: 13, margin: "8px 0 0", fontStyle: "italic" }}>{error}</p>}
          ...
          {result && meta && (
            <div style={{
              ...
              <button
                ...
                style={{
                  background: "white", border: `1.5px solid ${meta.border}`,
                  color: meta.color, borderRadius: 8, padding: "9px 16px",
        ...
        {/* MODE PILLS — icon field present on every object */}
        <div style={{
          display: "flex", gap: 10, flexWrap: "wrap",
          justifyContent: "center", marginBottom: 48, width: "100%",
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                transition: "all 0.2s", minWidth: 180, flex: 1,
                boxShadow: "0 1px 4px rgba(44,24,16,0.04)",
              }}
            >
              <div style={{ fontSize: 24, marginBottom: 8 }}>{m.icon}</div>
              <div style={{ color: "#2C1810", fontSize: 14, fontWeight: 500, marginBottom: 4 }}>{m.label}</div>
              <div style={{ color: "#8C7B6B", fontSize: 12, lineHeight: 1.5 }}>{m.sub}</div>
            </div>
          ))}
        </div>

        {/* STATS */}
        <div style={{ display: "flex", gap: 32, justifyContent: "center", animation: "fadeUp 0.7s 0.3s ease both" }}>
          {[["🔥","1,240+","learners active"],["⚡","50k+","videos processed"],["🪙","Coins","reward system"]].map(([icon,val,label]) => (
            <div key={label} style={{ textAlign: "center" }}>
              <div style={{ fontSize: 20, marginBottom: 4 }}>{icon}</div>
              <div style={{ color: "#2C1810", fontWeight: 600, fontSize: 15, fontFamily: "'DM Mono', monospace" }}>{val}</div>
              <div style={{ color: "#B5A898", fontSize: 11, marginTop: 2 }}>{label}</div>
            </div>
          ))}
        </div>
      </main>

      <footer style={{ textAlign: "center", padding: "20px", fontSize: 12, color: "#B5A898", borderTop: "1px solid var(--border)", fontStyle: "italic" }}>
